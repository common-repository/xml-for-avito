<?php
/**
 * Traits Voltage for variable products
 *
...
trait XFAVI_T_Variable_Get_Voltage {
	/**
	...
	public function get_voltage( $tag_name = 'Voltage', $result_xml = '' ) {
		...
		switch ( $voltage ) {
			...
			default:
				$voltage = (int) $voltage;
				$tag_value = $this->get_offer()->get_attribute( wc_attribute_taxonomy_name_by_id( $voltage ) );
				if ( empty( $tag_value ) ) {
		...
		$tag_value = apply_filters(
			'xfavi_f_variable_tag_value_voltage',
			$tag_value,
			[ 
		...
		if ( ! empty( $tag_value ) ) {
			$tag_name = apply_filters(
				'xfavi_f_variable_tag_name_voltage',
				$tag_name,
				[ 
					'product' => $this->get_product(),
					'offer' => $this->get_offer()
