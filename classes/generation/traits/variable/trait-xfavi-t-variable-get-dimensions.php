...
trait XFAVI_T_Variable_Get_Dimensions {
	/**
	 * Get dimensions tags
	 * 
	...
	public function get_dimensions( $tag_name = 'dimensions', $result_xml = '',
		...
		switch ( $length ) {
			...
			default:
				$length = (int) $length;
				$length_xml = $this->get_offer()->get_attribute( wc_attribute_taxonomy_name_by_id( $length ) );
				if ( empty( $length_xml ) ) {
		...
		switch ( $width ) {
			...
			default:
				$width = (int) $width;
				$width_xml = $this->get_offer()->get_attribute( wc_attribute_taxonomy_name_by_id( $width ) );
				if ( empty( $width_xml ) ) {
		...
		switch ( $height ) {
			...
			default:
				$height = (int) $height;
				$height_xml = $this->get_offer()->get_attribute( wc_attribute_taxonomy_name_by_id( $height ) );
				if ( empty( $height_xml ) ) {
		...
		$result_xml = apply_filters(
			'xfavi_f_variable_tag_dimensions',
			$result_xml,
			[ 
