<?php
/**
 * Traits Transmission_Spare_Part_Type for variable products
 *
...
trait XFAVI_T_Variable_Get_Transmission_Spare_Part_Type {
	/**
	...
	public function get_transmission_spare_part_type( $tag_name = 'TransmissionSparePartType', $result_xml = '' ) {
		...
		switch ( $transmission_spare_part_type ) {
			...
			default:
				...
				$tag_value = $this->get_offer()->get_attribute( wc_attribute_taxonomy_name_by_id( $transmission_spare_part_type ) );
				if ( empty( $tag_value ) ) {
		...
		$tag_value = apply_filters(
			'xfavi_f_variable_tag_value_transmission_spare_part_type',
			$tag_value,
			[ 
		...
		if ( ! empty( $tag_value ) ) {
			$tag_name = apply_filters(
				'xfavi_f_variable_tag_name_transmission_spare_part_type',
				$tag_name,
				[ 
					'product' => $this->get_product(),
					'offer' => $this->get_offer()
		...
		$result_xml = apply_filters(
			'xfavi_f_variable_tag_transmission_spare_part_type',
			$result_xml,
			[ 
				'product' => $this->get_product(),
				'offer' => $this->get_offer()
