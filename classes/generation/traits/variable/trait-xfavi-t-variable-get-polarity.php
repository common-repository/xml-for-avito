<?php
/**
 * Traits Polarity for variable products
 *
...
trait XFAVI_T_Variable_Get_Polarity {
	/**
	...
	public function get_polarity( $tag_name = 'Polarity', $result_xml = '' ) {
		...
		switch ( $polarity ) { /* disabled, sku, id */
			case "disabled": // выгружать штрихкод нет нужды		
				break;
			default:
				$polarity = (int) $polarity;
				$tag_value = $this->get_offer()->get_attribute( wc_attribute_taxonomy_name_by_id( $polarity ) );
				if ( empty( $tag_value ) ) {
		...
		$tag_value = apply_filters(
			'xfavi_f_variable_tag_value_polarity',
			$tag_value,
			[ 
		...
		if ( ! empty( $tag_value ) ) {
			$tag_name = apply_filters(
				'xfavi_f_variable_tag_name_polarity',
				$tag_name,
				[ 
					'product' => $this->get_product(),
					'offer' => $this->get_offer()
