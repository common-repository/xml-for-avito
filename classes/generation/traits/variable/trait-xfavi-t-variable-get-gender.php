<?php
/**
 * Traits Gender for variable products
 *
...
trait XFAVI_T_Variable_Get_Gender {
	/**
	...
	public function get_gender( $tag_name = 'Gender', $result_xml = '' ) {
		...
		switch ( $gender ) {
			...
			default:
				$gender = (int) $gender;
				$tag_value = $this->get_offer()->get_attribute( wc_attribute_taxonomy_name_by_id( $gender ) );
				if ( empty( $tag_value ) ) {
		...
		$tag_value = apply_filters(
			'xfavi_f_variable_tag_value_gender',
			$tag_value,
			[ 
		...
		if ( ! empty( $tag_value ) ) {
			$tag_name = apply_filters(
				'xfavi_f_variable_tag_name_gender',
				$tag_name,
				[ 
					'product' => $this->get_product(),
					'offer' => $this->get_offer()
