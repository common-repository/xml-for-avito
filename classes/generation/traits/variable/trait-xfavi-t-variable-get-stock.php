<?php
/**
 * Traits Stock for variable products
 *
...
trait XFAVI_T_Variable_Get_Stock {
	/**
	 * Get Stock
	 * 
	...
	public function get_stock( $tag_name = 'stock', $result_xml = '' ) {
		...
		if ( true == $this->get_offer()->get_manage_stock() ) { // включено управление запасом
			if ( $this->get_offer()->get_stock_quantity() > 0 ) {
				$tag_value = $this->get_offer()->get_stock_quantity();
			} else {
		...
		if ( ! empty( $tag_value ) ) {
			$tag_name = apply_filters(
				'xfavi_f_variable_tag_name_stock',
				$tag_name,
				[ 
