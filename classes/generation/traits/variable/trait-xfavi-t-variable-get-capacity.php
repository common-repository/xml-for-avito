<?php
/**
 * Traits Capacity for variable products
 *
...
trait XFAVI_T_Variable_Get_Capacity {
	/**
	...
	public function get_capacity( $tag_name = 'Capacity', $result_xml = '' ) {
		...
		switch ( $capacity ) {
			...
			default:
				...
				$tag_value = $this->get_offer()->get_attribute( wc_attribute_taxonomy_name_by_id( $capacity ) );
				if ( empty( $tag_value ) ) {
		...
		$tag_value = apply_filters(
			'xfavi_f_variable_tag_value_capacity',
			$tag_value,
			[ 
		...
		if ( ! empty( $tag_value ) ) {
			$tag_name = apply_filters(
				'xfavi_f_variable_tag_name_capacity',
				$tag_name,
				[ 
					'product' => $this->get_product(),
					'offer' => $this->get_offer()
