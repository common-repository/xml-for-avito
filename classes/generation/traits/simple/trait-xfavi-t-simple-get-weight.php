...
trait XFAVI_T_Simple_Get_Weight {
	/**
	 * Get weight tag
	 * 
	...
	public function get_weight( $tag_name = 'WeightForDelivery', $result_xml = '' ) {
		...
		if ( empty( $weight ) || $weight === 'woo_dimensions' ) {
			$weight_xml = $this->get_product()->get_weight(); // вес
			if ( ! empty( $weight_xml ) ) {
		...
		$tag_value = apply_filters(
			'xfavi_f_simple_tag_value_weight',
			$tag_value,
		...
		if ( ! empty( $tag_value ) ) {
			$tag_name = apply_filters(
				'xfavi_f_simple_tag_name_weight',
				$tag_name,
				[ 'product' => $this->get_product() ],
				$this->get_feed_id()
			);
