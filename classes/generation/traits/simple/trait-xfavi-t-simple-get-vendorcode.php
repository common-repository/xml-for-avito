...
trait XFAVI_T_Simple_Get_Vendorcode {
	/**
	...
	public function get_vendorcode( $tag_name = 'VendorCode', $result_xml = '' ) {
		...
		switch ( $xfavi_vendorcode ) { /* disabled, sku, post_meta, germanized, id */
			...
			default:
				$tag_value = apply_filters(
					'xfavi_f_simple_tag_value_switch_barcode',
					$tag_value,
					[ 
		...
		$tag_value = apply_filters(
			'xfavi_f_simple_tag_value_vendorcode',
			$tag_value,
			[ 
				'product' => $this->get_product()
			],
		...
		if ( ! empty( $tag_value ) ) {
			$tag_name = apply_filters(
				'xfavi_f_simple_tag_name_vendorcode',
				$tag_name,
				[ 
					'product' => $this->get_product()
				],
