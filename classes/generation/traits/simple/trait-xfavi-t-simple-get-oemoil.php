<?php
/**
 * Traits OEMOil for simple products
 *
...
trait XFAVI_T_Simple_Get_OEMOil {
	/**
	 * Get the `OEMOil` tag
	 * 
	...
	public function get_oemoil( $tag_name = 'OEMOil', $result_xml = '' ) {
		...
		$tag_value = apply_filters(
			'xfavi_f_simple_tag_value_oemoil',
			$tag_value,
		...
		if ( ! empty( $tag_value ) ) {
			$tag_name = apply_filters(
				'xfavi_f_simple_tag_name_oemoil',
				$tag_name,
				[ 'product' => $this->get_product() ],
				$this->get_feed_id()
			);
