		...
		public function print_view_html_feedback_block() { ?>
			<div class="postbox">
				...
				<div class="inside">
					<?php
					printf( '<p>%s! %s:</p>',
						__( 'Отправляя статистику, вы помогаете сделать плагин еще лучше', 'xml-for-avito' ),
						__( 'Следующие данные будут переданы', 'xml-for-avito' )
					);
					?>
					<ul class="icpd_ul">
						<?php
						printf( '<li>%s</li><li>%s</li><li>%s %s</li></p>',
							__( 'Информация о PHP версии', 'xml-for-avito' ),
							__( 'Включен ли режим multisite', 'xml-for-avito' ),
							__( 'Техническая информация и логи плагина', 'xml-for-avito' ),
					...
					<form action="<?php echo esc_url( $_SERVER['REQUEST_URI'] ); ?>" method="post" enctype="multipart/form-data">
						<?php
						printf( '<p>%s %s?</p>',
							__( 'Помог ли Вам плагин', 'xml-for-avito' ),
							esc_html( $this->get_plugin_name() )
						);
						?>
						<p>
							...
							printf( '<input type="radio" value="no" name="%s">%s<br />',
								esc_attr( $this->get_radio_name() ),
								__( 'Нет', 'xml-for-avito' )
							);
							?>
						</p>
						<p>
							<?php
							_e( "Введите свой адрес электронной почты, если вы хотите получить ответ",
								"xml-for-avito"
							); ?>:
						</p>
						...
						<p><textarea rows="6" cols="32" name="<?php echo $this->get_textarea_name(); ?>" placeholder="<?php
						   printf( '%1$s (%2$s). %3$s',
						   	__( 'Введите текст, чтобы отправить мне сообщение', 'xml-for-avito' ),
						   	__( 'Вы можете написать мне на русском или английском языке', 'xml-for-avito' ),
						   	__( 'Я проверяю свою электронную почту несколько раз в день', 'xml-for-avito' )
						   ); ?>"></textarea></p>
